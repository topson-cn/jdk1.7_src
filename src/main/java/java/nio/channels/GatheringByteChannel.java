 ...
 *
 * <p>
 * A <i>gathering</i> write operation writes, in a single invocation, a sequence of bytes from one
 * or more of a given sequence of buffers. Gathering writes are often useful when implementing
...
public interface GatheringByteChannel extends WritableByteChannel {
   ...
   *
   * <p>
   * An attempt is made to write up to <i>r</i> bytes to this channel, where <i>r</i> is the total
   * number of bytes remaining in the specified subsequence of the given buffer array, that is,
   *
   * <blockquote>
   * 
   * <pre>
   ...
   *
   * <p>
   * Suppose that a byte sequence of length <i>n</i> is written, where <tt>0</tt>&nbsp;
   * <tt>&lt;=</tt>&nbsp;<i>n</i>&nbsp;<tt>&lt;=</tt>&nbsp;<i>r</i>. Up to the first
   * <tt>srcs[offset].remaining()</tt> bytes of this sequence are written from buffer
   ...
   * some of the bytes or possibly none at all. A socket channel in non-blocking mode, for example,
   ...
   *
   * <p>
   * An invocation of this method of the form <tt>c.write(srcs)</tt> behaves in exactly the same
