/*
 * Copyright (c) 2007, 2011, Oracle and/or its affiliates. All rights reserved. ORACLE
 ...
 * {@link #position() <i>queried</i>} and {@link #position(long) <i>modified</i>}. The channel also
 * provides access to the current <i>size</i> of the entity to which the channel is connected. The
 * size increases when bytes are written beyond its current size; the size decreases when it is
 * {@link #truncate <i>truncated</i>}.
 *
 * <p>
 * The {@link #position(long) position} and {@link #truncate truncate} methods which do not
 * otherwise have a value to return are specified to return the channel upon which they are invoked.
 * This allows method invocations to be chained. Implementations of this interface should specialize
 * the return type so that method invocations on the implementation class can be chained.
 *
 * @since 1.7
...
public interface SeekableByteChannel extends ByteChannel {
   ...
   *
   * <p>
   * Bytes are written starting at this channel's current position, unless the channel is connected
   * to an entity such as a file that is opened with the
   * {@link java.nio.file.StandardOpenOption#APPEND APPEND} option, in which case the position is
   * first advanced to the end. The entity to which the channel is connected is grown, if necessary,
   * to accommodate the written bytes, and then the position is updated with the number of bytes
   * actually written. Otherwise this method behaves exactly as specified by the
   ...
   *
   * <p>
   * If the given size is less than the current size then the entity is truncated, discarding any
   * bytes beyond the new end. If the given size is greater than or equal to the current size then
   * the entity is not modified. In either case, if the current position is greater than the given
   * size then it is set to that size.
   *
   * <p>
   * An implementation of this interface may prohibit truncation when connected to an entity,
   * typically a file, opened with the {@link java.nio.file.StandardOpenOption#APPEND APPEND}
   * option.
   *
