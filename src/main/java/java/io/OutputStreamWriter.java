/*
 * Copyright (c) 1996, 2006, Oracle and/or its affiliates. All rights reserved. ORACLE
 ...
 * it are encoded into bytes using a specified {@link java.nio.charset.Charset <code>charset</code>}
 ...
 *
 * <p>
 * For top efficiency, consider wrapping an OutputStreamWriter within a BufferedWriter so as to
 * avoid frequent converter invocations. For example:
 *
 * <pre>
 ...
 *
 * <p>
 * A <i>surrogate pair</i> is a character represented by a sequence of two <tt>char</tt> values: A
 * <i>high</i> surrogate in the range '&#92;uD800' to '&#92;uDBFF' followed by a <i>low</i>
 * surrogate in the range '&#92;uDC00' to '&#92;uDFFF'.
 *
 * <p>
 * A <i>malformed surrogate element</i> is a high surrogate that is not followed by a low surrogate
 * or a low surrogate that is not preceded by a high surrogate.
 *
 * <p>
 * This class always replaces malformed surrogate elements and unmappable character sequences with
 * the charset's default <i>substitution sequence</i>. The
 * {@linkplain java.nio.charset.CharsetEncoder} class should be used when more control over the
...
public class OutputStreamWriter extends Writer {
   ...
   * passed to the constructor. This method may return <tt>null</tt> if the stream has been closed.
   * </p>
   *
  ...
  /**
   * Flushes the output buffer to the underlying byte stream, without flushing the byte stream
