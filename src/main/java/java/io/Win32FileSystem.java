/*
 * Copyright (c) 1998, 2010, Oracle and/or its affiliates. All rights reserved. ORACLE
...
class Win32FileSystem extends FileSystem {
  ...
  /*
   * A normal Win32 pathname contains no duplicate slashes, except possibly for a UNC prefix, and
   * does not end with a slash. It may be the empty string. Normalized Win32 pathnames have the
   * convenient property that the length of the prefix almost uniquely identifies the type of the
   * path and whether it is absolute or relative:
   * 
   * 0 relative to both drive and directory 1 drive-relative (begins with '\\') 2 absolute UNC (if
   * first char is '\\'), else directory-relative (has form "z:foo") 3 absolute local pathname
   * (begins with "z:\\")
   */
  ...
  private int normalizePrefix(String path, int len, StringBuffer sb) {
    ...
    if ((len - src >= 2) && isLetter(c = path.charAt(src)) && path.charAt(src + 1) == ':') {
      /*
       * Remove leading slashes if followed by drive specifier. This hack is necessary to support
       * file URLs containing drive specifiers (e.g., "file://c:/path"). As a side effect,
      ...
      if ((len >= 2) && isSlash(path.charAt(0)) && isSlash(path.charAt(1))) {
        /*
         * UNC pathname: Retain first slash; leave src pointed at second slash so that further
         * slashes will be collapsed into the second slash. The result will be a pathname beginning
         * with "\\\\" followed (most likely) by a host name.
         */
        src = 1;
        sb.append(slash);
      }
  ...
  /*
   * Normalize the given pathname, whose length is len, starting at the given offset; everything
   * before this offset is already normal.
   */
  private String normalize(String path, int len, int off) {
    ...
    /*
     * Remove redundant slashes from the remainder of the path, forcing all slashes into the
     * preferred slash
     */
    while (src < len) {
      ...
      if (isSlash(c)) {
        ...
        if (src == len) {
          ...
          if ((sn == 1) && (isSlash(sb.charAt(0)))) {
             ...
             * Even though it is not, by itself, a valid UNC pathname, we leave it as is in order to
             * be consistent with the win32 APIs, which treat this case as an invalid UNC pathname
  ...
  /*
   * Check that the given pathname is normal. If not, invoke the real normalizer on the part of the
   * pathname that requires normalization. This way we iterate through the whole pathname string
   * only once.
   */
  ...
  public String resolve(File f) {
    ...
    if (pl == 2) { /* Directory-relative */
      ...
      if (dir != null) {
        /*
         * When resolving a directory-relative path that refers to a drive other than the current
         * drive, insist that the caller have read permission on the result
         */
