 ...
 * strings</em> to name files and directories. This class presents an abstract, system-independent
 * view of hierarchical pathnames. An <em>abstract pathname</em> has two components:
 *
 * <ol>
 * <li>An optional system-dependent <em>prefix</em> string, such as a disk-drive specifier,
 * <code>"/"</code>&nbsp;for the UNIX root directory, or <code>"\\\\"</code>&nbsp;for a Microsoft
 ...
 *
 * <p>
 * A pathname, whether abstract or in string form, may be either <em>absolute</em> or
 ...
 * <code>java.io</code> package always resolve relative pathnames against the current user
 * directory. This directory is named by the system property <code>user.dir</code>, and is typically
 * the directory in which the Java virtual machine was invoked.
 *
 * <p>
 * The <em>parent</em> of an abstract pathname may be obtained by invoking the {@link #getParent}
 ...
 * <tt>File</tt> object with an absolute abstract pathname which begins with the directory's
 ...
 * consists of the drive letter followed by <code>":"</code> and possibly followed by
 * <code>"\\"</code> if the pathname is absolute. The prefix of a UNC pathname is
 * <code>"\\\\"</code>; the hostname and the share name are the first two names in the name
 ...
 * such as reading, writing, and executing. These restrictions are collectively known as <i>access
 * permissions</i>. The file system may have multiple sets of access permissions on a single object.
 * For example, one set may apply to the object's <i>owner</i>, and another may apply to all other
 * users. The access permissions on an object may cause some methods in this class to fail.
 *
 * <p>
 * Instances of the <code>File</code> class are immutable; that is, once created, the abstract
 * pathname represented by a <code>File</code> object will never change.
 *
 * <h4>Interoperability with {@code java.nio.file} package</h4>
 *
 * <p>
 * The <a href="../../java/nio/file/package-summary.html">{@code java.nio.file}</a> package defines
 * interfaces and classes for the Java virtual machine to access files, file attributes, and file
 * systems. This API may be used to overcome many of the limitations of the {@code java.io.File}
 * class. The {@link #toPath toPath} method may be used to obtain a {@link Path} that uses the
 * abstract path represented by a {@code File} object to locate a file. The resulting {@code Path}
 * may be used with the {@link java.nio.file.Files} class to provide more efficient and extensive
 * access to additional file operations, file attributes, and I/O exceptions to help diagnose errors
 * when an operation on a file fails.
 *
 * @author unascribed
 * @since JDK1.0
 */
...
public class File implements Serializable, Comparable<File> {
   ...
   * systems, this character is <code>':'</code>; on Microsoft Windows systems it is
   * <code>';'</code>.
   *
   * @see java.lang.System#getProperty(java.lang.String)
   */
   ...
   * disambiguate this method from the public(File, String) constructor.
   */
  ...
  /*
   * Note: The two-argument File constructors do not interpret an empty parent abstract pathname as
   * the current user directory. An empty parent instead causes the child to be resolved against the
   * system-dependent directory defined by the FileSystem.getDefaultParent method. On Unix this
   * default is "/", while on Microsoft Windows it is "\\". This is required for compatibility with
   * the original behavior of this class.
   */
   ...
   *
   * <p>
   * If <code>parent</code> is <code>null</code> then the new <code>File</code> instance is created
   * as if by invoking the single-argument <code>File</code> constructor on the given
   ...
   * a system-dependent way. If <code>parent</code> is the empty string then the new
   * <code>File</code> instance is created by converting <code>child</code> into an abstract
   ...
   *
   * <p>
   * The exact form of a <tt>file:</tt> URI is system-dependent, hence the transformation performed
   ...
   * name in the pathname's name sequence except for the last. If the name sequence is empty then
   ...
   * {@link #separator default name-separator character} to separate the names in the name sequence.
   *
   ...
   * dependent. On UNIX systems, a pathname is absolute if its prefix is <code>"/"</code>. On
   * Microsoft Windows systems, a pathname is absolute if its prefix is a drive specifier followed
   * by <code>"\\"</code>, or if its prefix is <code>"\\\\"</code>.
   *
   ...
   * if by the <code>{@link #getPath}</code> method. If this abstract pathname is the empty abstract
   * pathname then the pathname string of the current user directory, which is named by the system
   * property <code>user.dir</code>, is returned. Otherwise this pathname is resolved in a
   * system-dependent way. On UNIX systems, a relative pathname is made absolute by resolving it
   * against the current user directory. On Microsoft Windows systems, a relative pathname is made
   * absolute by resolving it against the current directory of the drive named by the pathname, if
   * any; if not, it is resolved against the current user directory.
   *
   ...
   *         security manager exists and its <code>{@link java.lang.SecurityManager#checkRead}
   *         </code> method denies read access to the file
   *
   * @since JDK1.1
   * @see Path#toRealPath
   */
   ...
   *
   * <p>
   * The exact form of the URI is system-dependent. If it can be determined that the file denoted by
   * this abstract pathname is a directory, then the resulting URI will end with a slash.
   *
   * <p>
   * For a given abstract pathname <i>f</i>, it is guaranteed that
   *
   ...
   *
   * <p>
   * Note that when this abstract pathname represents a UNC pathname then all components of the UNC
   * (including the server name component) are encoded in the {@code URI} path. The authority
   ...
   *
   * <p>
   * Where it is required to distinguish an I/O exception from the case where {@code 0L} is
   * returned, or where several attributes of the same file are required at the same time, or where
   * the time of last access or the creation time are required, then the
   * {@link java.nio.file.Files#readAttributes(Path,Class,LinkOption[]) Files.readAttributes} method
   * may be used.
   *
   ...
   *
   * <p>
   * Where it is required to distinguish an I/O exception from the case that {@code 0L} is returned,
   * or where several attributes of the same file are required at the same time, then the
   * {@link java.nio.file.Files#readAttributes(Path,Class,LinkOption[]) Files.readAttributes} method
   * may be used.
   *
   * @return The length, in bytes, of the file denoted by this abstract pathname, or <code>0L</code>
   *         if the file does not exist. Some operating systems may return <code>0L</code> for
   ...
   * <P>
   * Note: this method should <i>not</i> be used for file-locking, as the resulting protocol cannot
   * be made to work reliably. The {@link java.nio.channels.FileLock FileLock} facility should be
   * used instead.
   *
   ...
   *
   * @throws SecurityException If a security manager exists and its <code>
   *         {@link java.lang.SecurityManager#checkWrite(java.lang.String)}</code> method denies
   ...
   * file cannot be deleted. This is useful for error reporting and to diagnose why a file cannot be
   * deleted.
   *
   ...
   *
   * @throws SecurityException If a security manager exists and its <code>
   *         {@link java.lang.SecurityManager#checkDelete}</code> method denies delete access to the
   *         file
   */
   ...
   * directory and iterate over the names of the files in the directory. This may use less resources
   * when working with very large directories, and may be more responsive when working with remote
   * directories.
   *
   ...
   *         {@link SecurityManager#checkRead(String)} method denies read access to the directory
   */
  public String[] list() {
    SecurityManager security = System.getSecurityManager();
    if (security != null) {
   ...
   * a name satisfies the filter if and only if the value {@code true} results when the
   * {@link FilenameFilter#accept FilenameFilter.accept(File,&nbsp;String)} method of the filter is
   ...
   *
   * @return An array of strings naming the files and directories in the directory denoted by this
   *         abstract pathname that were accepted by the given {@code filter}. The array will be
   ...
   * directory. Pathnames denoting the directory itself and the directory's parent directory are not
   * included in the result. Each resulting abstract pathname is constructed from this abstract
   * pathname using the {@link #File(File, String) File(File,&nbsp;String)} constructor. Therefore
   * if this pathname is absolute then each resulting pathname is absolute; if this pathname is
   ...
   * {@link java.nio.file.Files#newDirectoryStream(Path) newDirectoryStream} method to open a
   * directory and iterate over the names of the files in the directory. This may use less resources
   * when working with very large directories.
   *
   ...
   * {@code true} results when the {@link FilenameFilter#accept
   ...
   * <code>{@link #lastModified}</code> method will return the (possibly truncated)
   ...
   * allowed. After invoking this method the file or directory is guaranteed not to change until it
   * is either deleted or marked to allow write access. Whether or not a read-only file or directory
   ...
   *
   * <p>
   * The {@link java.nio.file.Files} class defines methods that operate on file attributes including
   * file permissions. This may be used when finer manipulation of file permissions is required.
   *
   ...
   *
   * <p>
   * An invocation of this method of the form <tt>file.setWritable(arg)</tt> behaves in exactly the
   ...
   *        permission; otherwise, it applies to everybody. If the underlying file system can not
   *        distinguish the owner's read permission from that of others, then the permission will
   *        apply to everybody, regardless of this value.
   *
   * @return <code>true</code> if and only if the operation succeeded. The operation will fail if
   *         the user does not have permission to change the access permissions of this abstract
   ...
   *
   * <p>
   * An invocation of this method of the form <tt>file.setReadable(arg)</tt> behaves in exactly the
   * same way as the invocation
   *
   * <pre>
   ...
   *        execute permission; otherwise, it applies to everybody. If the underlying file system
   *        can not distinguish the owner's execute permission from that of others, then the
   *        permission will apply to everybody, regardless of this value.
   *
   * @return <code>true</code> if and only if the operation succeeded. The operation will fail if
   *         the user does not have permission to change the access permissions of this abstract
   *         pathname. If <code>executable</code> is <code>false</code> and the underlying file
   *         system does not implement an execute permission, then the operation will fail.
   *
   * @throws SecurityException If a security manager exists and its <code>
   *         {@link java.lang.SecurityManager#checkWrite(java.lang.String)}</code> method denies
   *         write access to the file
   ...
   *
   * <p>
   * An invocation of this method of the form <tt>file.setExcutable(arg)</tt> behaves in exactly the
   * same way as the invocation
   *
   * <pre>
   ...
   *         the user does not have permission to change the access permissions of this abstract
   *         pathname. If <code>executable</code> is <code>false</code> and the underlying file
   *         system does not implement an excute permission, then the operation will fail.
   *
   * @throws SecurityException If a security manager exists and its <code>
   *         {@link java.lang.SecurityManager#checkWrite(java.lang.String)}</code> method denies
   *         write access to the file
   ...
   *
   * @throws SecurityException If a security manager exists and its <code>{@link
   *          java.lang.SecurityManager#checkExec(java.lang.String)}</code> method denies execute
   *         access to the file
   *
   * @since 1.6
   */
   ...
   * file system has a {@code root} directory from which all other files in that file system can be
   * reached. Windows platforms, for example, have a root directory for each active drive; UNIX
   * platforms have a single root directory, namely {@code "/"}. The set of available filesystem
   * roots is affected by various system-level operations such as the insertion or ejection of
   * removable media and the disconnecting or unmounting of physical or virtual disk drives.
   *
   * <p>
   * This method returns an array of {@code File} objects that denote the root directories of the
   * available filesystem roots. It is guaranteed that the canonical pathname of any file physically
   * present on the local machine will begin with one of the roots returned by this method.
   *
   * <p>
   * The canonical pathname of a file that resides on some other machine and is accessed via a
   * remote-filesystem protocol such as SMB or NFS may or may not begin with one of the roots
   * returned by this method. If the pathname of a remote file is syntactically indistinguishable
   * from the pathname of a local file then it will begin with one of the roots returned by this
   * method. Thus, for example, {@code File} objects denoting the root directories of the mapped
   * network drives of a Windows platform will be returned by this method, while {@code File}
   * objects containing UNC pathnames will not be returned by this method.
   *
   * <p>
   * Unlike most methods in this class, this method does not throw security exceptions. If a
   * security manager exists and its {@link SecurityManager#checkRead(String)} method denies read
   * access to a particular root directory, then that directory will not appear in the result.
   *
   * @return An array of {@code File} objects denoting the available filesystem roots, or
   *         {@code null} if the set of roots could not be determined. The array will be empty if
   *         there are no filesystem roots.
   *
   * @since 1.2
   ...
   *         {@link RuntimePermission}<tt>("getFileSystemAttributes")</tt> or its
   *         {@link SecurityManager#checkRead(String)} method denies read access to the file named
   ...
   *
   * <p>
   * The returned number of unallocated bytes is a hint, but not a guarantee, that it is possible to
   * use most or any of these bytes. The number of unallocated bytes is most likely to be accurate
   * immediately after this call. It is likely to be made inaccurate by any external I/O operations
   * including those made on the system outside of this virtual machine. This method makes no
   ...
   *         pathname does not name a partition. This value will be less than or equal to the total
   *         file system size returned by {@link #getTotalSpace}.
   *
   * @throws SecurityException If a security manager has been installed and it denies
   *         {@link RuntimePermission}<tt>("getFileSystemAttributes")</tt> or its
   *         {@link SecurityManager#checkRead(String)} method denies read access to the file named
   ...
   *         does not name a partition. On systems where this information is not available, this
   *         method will be equivalent to a call to {@link #getFreeSpace}.
   *
   * @throws SecurityException If a security manager has been installed and it denies
   *         {@link RuntimePermission}<tt>("getFileSystemAttributes")</tt> or its
   *         {@link SecurityManager#checkRead(String)} method denies read access to the file named
   ...
   * invoked, and
   * <li>Neither this method nor any of its variants will return the same abstract pathname again in
   * the current invocation of the virtual machine.
   * </ol>
   *
   ...
   *
   * <p>
   * The <code>prefix</code> argument must be at least three characters long. It is recommended that
   * the prefix be a short, meaningful string such as <code>"hjb"</code> or <code>"mail"</code>. The
   * <code>suffix</code> argument may be <code>null</code>, in which case the suffix
   ...
   * truncated, but if it begins with a period character (<code>'.'</code>) then the period and the
   * first three characters following it will always be preserved. Once these adjustments have been
   * made the name of the new file will be generated by concatenating the prefix, five or more
   ...
   * temporary-file directory will be used. The default temporary-file directory is specified by the
   * system property <code>java.io.tmpdir</code>. On UNIX systems the default value of this property
   * is typically <code>"/tmp"</code> or <code>"/var/tmp"</code>; on Microsoft Windows systems it is
   * typically <code>"C:\\WINNT\\TEMP"</code>. A different value may be given to this system
   * property when the Java virtual machine is invoked, but programmatic changes to this property
   * are not guaranteed to have any effect upon the temporary directory used by this method.
   *
   ...
   *
   * @throws SecurityException If a security manager exists and its <code>
   *         {@link java.lang.SecurityManager#checkWrite(java.lang.String)}</code> method does not
   ...
   * <code>{@link #createTempFile(java.lang.String,
   ...
   *         abstract pathname is lexicographically less than the argument, or a value greater than
   *         zero if this abstract pathname is lexicographically greater than the argument
   *
   * @since 1.2
   */
  ...
  /**
   * WriteObject is called to save this filename. The separator character is saved also so it can be
   * replaced in case the path is reconstituted on a different host type.
   * <p>
   * 
  ...
  /**
   * readObject is called to restore this filename. The original separator character is read. If it
   * is different than the separator character on this system, then the old separator is replaced by
   * the local separator.
   */
   ...
   * </pre>
   * 
   * </blockquote> Subsequent invocations of this method return the same {@code Path}.
   *
   * <p>
