 ...
 * <code>PipedOutputStream</code> object by one thread and data is read from the connected
 * <code>PipedInputStream</code> by some other thread. Attempting to use both objects from a single
 * thread is not recommended as it may deadlock the thread. The pipe is said to be <a name=BROKEN>
 * <i>broken</i> </a> if a thread that was reading data bytes from the connected piped input stream
 * is no longer alive.
 *
...
public class PipedOutputStream extends OutputStream {
   ...
   * thread groups (but what about pipes within a thread?) or using finalization (but it may be a
   ...
   * <p>
   * If <code>snk</code> is an unconnected piped input stream and <code>src</code> is an unconnected
   * piped output stream, they may be connected by either the call: <blockquote>
   * 
   * <pre>
   * src.connect(snk)
   * </pre>
   ...
   *            {@link #connect(java.io.PipedInputStream) unconnected}, closed, or if an I/O error
   *            occurs.
   */
