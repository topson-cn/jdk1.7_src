 ...
 * platform's default character encoding. The <code>{@link
 * PrintWriter}</code> class should be used in situations that require writing characters rather
 * than bytes.
 *
 * @author Frank Yellin
...
public class PrintStream extends FilterOutputStream implements Appendable, Closeable {
  ...
  /**
   * Track both the text- and character-output streams, so that their buffers can be flushed without
   * flushing the entire stream.
   */
  private BufferedWriter textOut;
  private OutputStreamWriter charOut;

  /**
   * requireNonNull is explicitly declared here so as not to create an extra dependency on
   * java.util.Objects.requireNonNull. PrintStream is loaded early during system initialization.
   */
  ...
  /*
   * Variant of the private constructor so that the given charset name can be verified before
   * evaluating the OutputStream argument. Used by constructors creating a FileOutputStream that
   * also take a charset name.
   */
   ...
   *        written, one of the <code>println</code> methods is invoked, or a newline character or
   *        byte (<code>'\n'</code>) is written
   *
  ...
  /**
   * Creates a new print stream, without automatic line flushing, with the specified file name. This
  ...
  /**
   * Creates a new print stream, without automatic line flushing, with the specified file name and
  ...
  /**
   * Creates a new print stream, without automatic line flushing, with the specified file. This
   * convenience constructor creates the necessary intermediate {@link java.io.OutputStreamWriter
   * OutputStreamWriter}, which will encode characters using the
   * {@linkplain java.nio.charset.Charset#defaultCharset() default charset} for this instance of the
   * Java virtual machine.
   ...
   *        it will be truncated to zero size; otherwise, a new file will be created. The output
  ...
  /**
   * Creates a new print stream, without automatic line flushing, with the specified file and
   * charset. This convenience constructor creates the necessary intermediate
   * {@link java.io.OutputStreamWriter OutputStreamWriter}, which will encode characters using the
   * provided charset.
   *
   ...
   *         error occurs while opening or creating the file
   *
   * @throws SecurityException If a security manager is presentand
   *         {@link SecurityManager#checkWrite checkWrite(file.getPath())} denies write access to
   *         the file
   *
   * @throws UnsupportedEncodingException If the named charset is not supported
   ...
   * than <code>InterruptedIOException</code>, and when the <code>setError</code> method is invoked.
   * If an operation on the underlying output stream throws an <code>InterruptedIOException</code>,
   * then the <code>PrintStream</code> converts the exception back into an interrupt by doing:
   * 
   * <pre>
   ...
   *
   * <p>
   * Note that the byte is written as given; to write a character that will be translated according
   * to the platform's default character encoding, use the <code>print(char)</code> or
  ...
  /*
   * The following private methods on the text- and character-output streams always flush the stream
   * buffers, so that writes to the underlying byte stream occur as promptly as with the original
   * PrintStream.
   */
   ...
   * the entire sequence may not be appended. For instance, invoking then <tt>toString</tt> method
   * of a character buffer will return a subsequence whose content depends upon the buffer's
   * position and limit.
   *
