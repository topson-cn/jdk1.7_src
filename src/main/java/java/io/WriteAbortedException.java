...
/**
 * Signals that one of the ObjectStreamExceptions was thrown during a write operation. Thrown during
 * a read operation when one of the ObjectStreamExceptions was thrown during a write operation. The
 * exception that terminated the write can be found in the detail field. The stream is reset to it's
 ...
 * exception-chaining mechanism. The "exception causing the abort" that is provided at construction
 * time and accessed via the public {@link #detail} field is now known as the <i>cause</i>, and may
