 ...
 * the underlying character or byte stream. It is therefore advisable to wrap a BufferedReader
...
public class BufferedReader extends Reader {
  ...
  private int read1(char[] cbuf, int off, int len) throws IOException {
    if (nextChar >= nChars) {
       ...
       * activity, and if line feeds are not being skipped, do not bother to copy the characters
       * into the local buffer. In this way buffered streams will cascade harmlessly.
       */
   ...
   * <code>{@link Reader}</code> class. As an additional convenience, it attempts to read as many
   * characters as possible by repeatedly invoking the <code>read</code> method of the underlying
   ...
   *
   * <p>
   * Ordinarily this method takes characters from this stream's character buffer, filling it from
   ...
   * characters directly from the underlying stream into the given array. Thus redundant
  ...
  public boolean ready() throws IOException {
    synchronized (lock) {
      ...
      /*
       * If newline needs to be skipped and the next char to be read is a newline character, then
       * just skip it right away.
       */
      if (skipLF) {
        /*
         * Note that in.ready() will return true if and only if the next read on the stream will not
         * block.
         */
