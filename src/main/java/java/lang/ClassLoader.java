...
public abstract class ClassLoader
{
	...
	private ClassLoader(Void unused, ClassLoader parent)
	{
		...
		if (ParallelLoaders.isRegistered(this.getClass()))
		{
			...
			domains = Collections
					.synchronizedSet(new HashSet<ProtectionDomain>());
	 ...
	 * name. Otherwise, the method returns this ClassLoader object. </p>
	 *
	 * @param className
	...
	private Class defineTransformedClass(String name, byte[] b, int off,
			int len, ProtectionDomain pd, ClassFormatError cfe, String source)
			throws ClassFormatError
	{
		// Class format error - try to transform the bytecode and
		// define the class again
		//
		ClassFileTransformer[] transformers = ClassFileTransformer
				.getTransformers();
		Class c = null;
	 ...
	 * the byte array "<tt>b</tt>", otherwise a {@link <tt>NoClassDefFoundError</tt>}
	 * will be thrown.
	 * </p>
	 *
	 * @param name
		...
		if (!b.isDirect())
		{
			if (b.hasArray())
			{
				return defineClass(name, b.array(),
						b.position() + b.arrayOffset(), len, protectionDomain);
			}
			else
			{
	...
	/**
	 * Registers the caller as parallel capable.</p> The registration succeeds
	 * if and only if all of the following conditions are met: <br>
	 * 1. no instance of the caller has been created</p> 2. all of the super
	 * classes (except class Object) of the caller are registered as parallel
	 * capable</p> Note that once a class loader is registered as parallel
	 * capable, there is no way to change it back. </p>
	 *
	 ...
	 * (see {@link #getSystemClassLoader()}). </p>
	 *
	 * @param name
	 *            The resource name
	 *
	 ...
	 * {@link java.util.Enumeration <tt>Enumeration</tt>} of
	 * {@link java.net.URL <tt>URL</tt>} objects.
	 *
	 * <p>
	 * The search order is described in the documentation for
	 ...
	 * class loader (see {@link #getSystemClassLoader()}). </p>
	 *
	 * @param name
	 *            The resource name
	 *
			...
			if (l != null)
			{
				...
				try
				{
					scl = AccessController
							.doPrivileged(new SystemClassLoaderAction(scl));
				}
	...
	{
		ClassLoader loader = (fromClass == null) ? null : fromClass
				.getClassLoader();
		if (sys_paths == null)
		{
	...
	{
		boolean exists = AccessController
	 ...
	 * status settings and "start with a clean slate." </p>
	 *
	 * @since 1.4
	 */
	 ...
	 * class loader's default assertion status is returned. </p>
	 *
	 * @param className
...
class SystemClassLoaderAction implements PrivilegedExceptionAction<ClassLoader>
{
	...
	public ClassLoader run() throws Exception
	{
		...
		ClassLoader sys = (ClassLoader) ctor
