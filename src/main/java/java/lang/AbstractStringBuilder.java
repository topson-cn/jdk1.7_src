...
abstract class AbstractStringBuilder implements Appendable, CharSequence
{
	 ...
	 * every nonnegative index <i>k</i> less than <code>newLength</code>, the
	 ...
	 * sequence, and the <code>char</code> value at the following index is in
	 * the low-surrogate range, then the supplementary code point corresponding
	 * to this surrogate pair is returned. Otherwise, the <code>char</code>
	 ...
	 * ranges from <code>1</code> to {@link #length()}.
	 *
	 * <p>
	 ...
	 * this sequence. The text range begins at the specified
	 ...
	 * within this sequence count as one code point each.
	 *
	 * @param beginIndex
	 ...
	 *                sequence, or <code>beginIndex</code> is larger than
	 *                <code>endIndex</code>.
	 */
	 ...
	 * <code>index</code> by <code>codePointOffset</code> code points. Unpaired
	 * surrogates within the text range given by <code>index</code> and
	 ...
	 *                length of this sequence, or if
	 *                <code>codePointOffset</code> is positive and the
	 *                subsequence starting with <code>index</code> has fewer
	 *                than <code>codePointOffset</code> code points, or if
	 *                <code>codePointOffset</code> is negative and the
	 *                subsequence before <code>index</code> has fewer than the
	 *                absolute value of <code>codePointOffset</code> code
	 *                points.
	 */
	 ...
	 * array <code>dst</code>. The first character to be copied is at index
	 * <code>srcBegin</code>; the last character to be copied is at index
	...
	public AbstractStringBuilder append(int i)
	{
		...
		int appendedLength = (i < 0) ? Integer.stringSize(-i) + 1 : Integer
				.stringSize(i);
	...
	public AbstractStringBuilder append(long l)
	{
		...
		int appendedLength = (l < 0) ? Long.stringSize(-l) + 1 : Long
				.stringSize(l);
		int spaceNeeded = count + appendedLength;
		ensureCapacityInternal(spaceNeeded);
	 ...
	 * This sequence is shortened by one <code>char</code>.
	 *
	 * <p>
	 ...
	 * <code>char</code>s to remove by calling
	 ...
	 * in the specified <code>String</code>. The substring begins at the
	 * specified <code>start</code> and extends to the character at index
	 * <code>end - 1</code> or to the end of the sequence if no such character
	 * exists. First the characters in the substring are removed and then the
	 * specified <code>String</code> is inserted at <code>start</code>. (This
	 * sequence will be lengthened to accommodate the specified String if
	 * necessary.)
	 *
	 * @param start
	 *            The beginning index, inclusive.
	 ...
	 * characters currently contained in this character sequence. The substring
	 * begins at the specified index and extends to the end of this sequence.
	 *
	 * @param start
	 *            The beginning index, inclusive.
	 * @return The new string.
	 * @throws StringIndexOutOfBoundsException
	 *             if <code>start</code> is less than zero, or greater than the
	...
	/**
	 * Returns a new <code>String</code> that contains a subsequence of
	 * characters currently contained in this sequence. The substring begins at
	 * the specified <code>start</code> and extends to the character at index
	 * <code>end - 1</code>.
	 *
	 * @param start
	 *            The beginning index, inclusive.
	 ...
	 *             greater than <code>length()</code>, or <code>start</code> is
	 ...
	 * length in <code>char</code> values) just prior to execution of the
	 * <code>reverse</code> method. Then the character at index <i>k</i> in the
	 * new character sequence is equal to the character at index <i>n-k-1</i> in
	 ...
	 * <code>String</code> is then returned. Subsequent changes to this sequence
	 * do not affect the contents of the <code>String</code>.
	 *
