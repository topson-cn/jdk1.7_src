 ...
 * characters translated to uppercase or to lowercase. Case mapping is based on
 * the Unicode Standard version specified by the {@link java.lang.Character
 * Character} class.
 * <p>
 ...
 * String concatenation is implemented through the <code>StringBuilder</code>(or
 * <code>StringBuffer</code>) class and its <code>append</code> method. String
 * conversions are implemented through the method <code>toString</code>, defined
 * by <code>Object</code> and inherited by all classes in Java. For additional
 ...
 *
 * <p>
 * A <code>String</code> represents a string in the UTF-16 format in which
 * <em>supplementary characters</em> are represented by <em>surrogate
 * pairs</em> (see the section <a href="Character.html#unicode">Unicode
 * Character Representations</a> in the <code>Character</code> class for more
 * information). Index values refer to <code>char</code> code units, so a
 * supplementary character uses two positions in a <code>String</code>.
 * <p>
 * The <code>String</code> class provides methods for dealing with Unicode code
 * points (i.e., characters), in addition to those for dealing with Unicode code
 * units (i.e., <code>char</code> values).
 *
 * @author Lee Boynton
		...
		CharSequence
{
	 ...
	 * handle is generated to refer to all future references to the string
	 * instance within the stream.
	 */
	 ...
	 * Obtaining a string from a string builder via the {@code toString} method
	 * is likely to run faster and is generally preferred.
	 *
	 * @param builder
	 ...
	 * index refers to <code>char</code> values (Unicode code units) and ranges
	 * from <code>0</code> to {@link #length()}<code> - 1</code>.
	 *
	 * <p>
	 * If the <code>char</code> value specified at the given index is in the
	 * high-surrogate range, the following index is less than the length of this
	 * <code>String</code>, and the <code>char</code> value at the following
	 ...
	 *
	 * @param index
	 *            the index to the <code>char</code> values
	 ...
	 * The index refers to <code>char</code> values (Unicode code units) and
	 * ranges from <code>1</code> to {@link CharSequence#length() length}.
	 *
	 * <p>
	 * If the <code>char</code> value at <code>(index - 1)</code> is in the
	 * low-surrogate range, <code>(index - 2)</code> is not negative, and the
	 * <code>char</code> value at <code>(index -
	 * 2)</code> is in the high-surrogate range, then the supplementary code
	 * point value of the surrogate pair is returned. If the <code>char</code>
	 * value at <code>index -
	 * 1</code> is an unpaired low-surrogate or a high-surrogate, the surrogate
	 * value is returned.
	 *
	 * @param index
	 ...
	 * @exception IndexOutOfBoundsException
	 *                if the <code>index</code> argument is less than 1 or
	 ...
	 * this <code>String</code>. The text range begins at the specified
	 * <code>beginIndex</code> and extends to the <code>char</code> at index
	 * <code>endIndex - 1</code>. Thus the length (in <code>char</code>s) of the
	 * text range is <code>endIndex-beginIndex</code>. Unpaired surrogates
	 * within the text range count as one code point each.
	 *
	 * @param beginIndex
	 *            the index to the first <code>char</code> of the text range.
	 * @param endIndex
	 *            the index after the last <code>char</code> of the text range.
	 ...
	 *                <code>String</code>, or <code>beginIndex</code> is larger
	...
	/**
	 * Returns the index within this <code>String</code> that is offset from the
	 * given <code>index</code> by <code>codePointOffset</code> code points.
	 * Unpaired surrogates within the text range given by <code>index</code> and
	 ...
	 * @exception IndexOutOfBoundsException
	 *                if <code>index</code> is negative or larger then the
	 *                length of this <code>String</code>, or if
	 *                <code>codePointOffset</code> is positive and the substring
	 *                starting with <code>index</code> has fewer than
	 *                <code>codePointOffset</code> code points, or if
	 *                <code>codePointOffset</code> is negative and the substring
	 *                before <code>index</code> has fewer than the absolute
	 *                value of <code>codePointOffset</code> code points.
	 * @since 1.5
	 */
	 ...
	 * converted to bytes, are copied into the subarray of {@code dst} starting
	 ...
	 *             If any of the following is true:
	 *             <ul>
	 *             <li> {@code srcBegin} is negative <li> {@code srcBegin} is
	 *             greater than {@code srcEnd} <li> {@code srcEnd} is greater
	 *             than the length of this String <li> {@code dstBegin} is
	 *             negative <li> {@code dstBegin+(srcEnd-srcBegin)} is larger
	 ...
	 * identical character sequences. The substring of this <tt>String</tt>
	 * object to be compared begins at index <tt>toffset</tt> and has length
	 * <tt>len</tt>. The substring of other to be compared begins at index
	 * <tt>ooffset</tt> and has length <tt>len</tt>. The result is
	 * <tt>false</tt> if and only if at least one of the following is true:
	 * <ul>
	 ...
	 * this string, then <code>-1</code> is returned.
	 *
	 * @param ch
	 ...
	 * occurrence is returned. For values of <code>ch</code> in the range from 0
	 * to 0xFFFF (inclusive), this is the smallest value <i>k</i> such that:
	 * <blockquote>
	 * 
	 * <pre>
	 ...
	 * 
	 * </blockquote> is true. In either case, if no such character occurs in
	 * this string at or after position <code>fromIndex</code>, then
	 ...
	 * specified character. For values of <code>ch</code> in the range from 0 to
	 ...
	 * 
	 * </blockquote> is true. In either case, if no such character occurs in
	 * this string, then <code>-1</code> is returned. The <code>String</code> is
	 ...
	 *
	 * <p>
	 * An invocation of this method of the form <i>str</i><tt>.matches(</tt>
	 * <i>regex</i><tt>)</tt> yields exactly the same result as the expression
	 *
	 ...
	 *
	 * @return <tt>true</tt> if, and only if, this string matches the given
	 *         regular expression
	 *
	...
	public String toLowerCase(Locale locale)
	{
		...
		int resultOffset = 0; /*
							 * result may grow, so i+resultOffset is the write
							 * location in result
							 */
				...
				if (lowerChar == Character.ERROR)
				{
					if (!localeDependent && srcChar == '\u0130')
					{
						lowerCharArray = ConditionalSpecialCasing
								.toLowerCaseCharArray(this, i, Locale.ENGLISH);
					}
					else
					{
						lowerCharArray = ConditionalSpecialCasing
	 ...
	 * keys, and HTML tags. For instance, <code>"TITLE".toLowerCase()</code> in
	 * a Turkish locale returns <code>"t\u005Cu0131tle"</code>, where
	 * '\u005Cu0131' is the LATIN SMALL LETTER DOTLESS I character. To obtain
	 * correct results for locale insensitive strings, use
		...
		for (int i = firstLower; i < len; i += srcCount)
		{
					...
					|| (upperChar >= Character.MIN_SUPPLEMENTARY_CODE_POINT))
			{
				if (upperChar == Character.ERROR)
				{
					...
					{
						upperCharArray = Character
								.toUpperCaseCharArray(srcChar);
					}
				}
				else if (srcCount == 2)
				{
	 ...
	 * results if used for strings that are intended to be interpreted locale
	 * independently. Examples are programming language identifiers, protocol
	 * keys, and HTML tags. For instance, <code>"title".toUpperCase()</code> in
	 * a Turkish locale returns <code>"T\u005Cu0130TLE"</code>, where
	 * '\u005Cu0130' is the LATIN CAPITAL LETTER I WITH DOT ABOVE character. To
	 ...
	 * @param c
	 *            a <code>char</code>.
	 * @return a string of length <code>1</code> containing as its single
	...
	static
	{
		...
		int SEED_MATERIAL[] = { System.identityHashCode(String.class),
				System.identityHashCode(System.class), (int) (nanos >>> 32),
				(int) nanos, (int) (now >>> 32), (int) now,
