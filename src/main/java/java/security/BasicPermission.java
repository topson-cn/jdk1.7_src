...
public abstract class BasicPermission extends Permission implements java.io.Serializable {
   ...
   *
   * <p>
   * BasicPermission objects must be stored in a manner that allows them to be inserted in any
   * order, but that also enables the PermissionCollection <code>implies</code> method to be
  ...
  /**
   * Returns the canonical name of this BasicPermission. All internal invocations of getName should
   * invoke this method, so that the pre-JDK 1.6 "exitVM" and current "exitVM.*" permission are
...
/**
 * A BasicPermissionCollection stores a collection of BasicPermission permissions. BasicPermission
 * objects must be stored in a manner that allows them to be inserted in any order, but enable the
 * implies function to evaluate the implies method in an efficient (and consistent) manner.
 *
 * A BasicPermissionCollection handles comparing a permission like "a.b.c.d.e" with a Permission
 * such as "a.b.*", or "*".
 *
 * @see java.security.Permission
 * @see java.security.Permissions
...
final class BasicPermissionCollection extends PermissionCollection implements java.io.Serializable {
  ...
  /**
   * Key is name, value is permission. All permission objects in collection must be of the same
   * type. Not serialized; see serialization section at end of class.
   */
  private transient Map<String, Permission> perms;

  /**
   * This is set to <code>true</code> if this BasicPermissionCollection contains a BasicPermission
   ...
   *
   * @exception IllegalArgumentException - if the permission is not a BasicPermission, or if the
   *            permission is not of the same Class as the other permissions in this collection.
   *
   * @exception SecurityException - if this BasicPermissionCollection object has been marked
   *            readonly
   */
   ...
   *              BasicPermissionCollection. All BasicPermissions in the collection must belong to
   *              the same class. The Hashtable is indexed by the BasicPermission name; the value of
   *              the Hashtable entry is the permission.
   * @serialField all_allowed boolean This is set to <code>true</code> if this
   *              BasicPermissionCollection contains a BasicPermission with '*' as its permission
   *              name.
   ...
   */
  /*
   * Writes the contents of the perms field out as a Hashtable for serialization compatibility with
   * earlier releases. all_allowed and permClass unchanged.
   */
