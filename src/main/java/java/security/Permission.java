/*
 * Copyright (c) 1997, 2009, Oracle and/or its affiliates. All rights reserved. ORACLE
...
public abstract class Permission implements Guard, java.io.Serializable {
   ...
   * <code>SecurityManager.checkPermission</code> method is called, passing this permission object
   * as the permission to check. Returns silently if access is granted. Otherwise, throws a
   * SecurityException.
   *
   ...
   *
   * <p>
   * The <code>implies</code> method is used by the AccessController to determine whether or not a
   * requested permission is implied by another permission that is known to be valid in the current
   * execution context.
   *
   ...
   * <p>
   * <ul>
   * <li>Whenever it is invoked on the same Permission object more than once during an execution of
   * a Java application, the <code>hashCode</code> method must consistently return the same integer.
   ...
   * calling the <code>hashCode</code> method on each of the two Permission objects must produce the
   * same integer result.
   * </ul>
   *
  ...
  /**
   * Returns an empty PermissionCollection for a given Permission object, or null if one is not
   * defined. Subclasses of class Permission should override this if they need to store their
   * permissions in a particular PermissionCollection object in order to provide the correct
   * semantics when the <code>PermissionCollection.implies</code> method is called. If null is
   * returned, then the caller of this method is free to store permissions of this type in any
