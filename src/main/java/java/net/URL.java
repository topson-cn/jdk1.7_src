/*
 * Copyright (c) 1995, 2008, Oracle and/or its affiliates. All rights reserved. ORACLE
...
/**
 * Class <code>URL</code> represents a Uniform Resource Locator, a pointer to a "resource" on the
 * World Wide Web. A resource can be something as simple as a file or a directory, or it can be a
 * reference to a more complicated object, such as a query to a database or to a search engine. More
 * information on the types of URLs and their formats can be found at: <blockquote> <a
 * href="http://www.socs.uts.edu.au/MosaicDocs-old/url-primer.html">
 * <i>http://www.socs.uts.edu.au/MosaicDocs-old/url-primer.html</i></a> </blockquote>
 * <p>
 * In general, a URL can be broken into several parts. The previous example of a URL indicates that
 * the protocol to use is <code>http</code> (HyperText Transfer Protocol) and that the information
 * resides on a host machine named <code>www.socs.uts.edu.au</code>. The information on that host
 * machine is named <code>/MosaicDocs-old/url-primer.html</code>. The exact meaning of this name on
 * the host machine is both protocol dependent and host dependent. The information normally resides
 * in a file, but it could be generated on the fly. This component of the URL is called the
 * <i>path</i> component.
 * <p>
 * A URL can optionally specify a "port", which is the port number to which the TCP connection is
 * made on the remote host machine. If the port is not specified, the default port for the protocol
 * is used instead. For example, the default port for <code>http</code> is <code>80</code>. An
 * alternative port could be specified as: <blockquote>
 * 
 * <pre>
 ...
 * href="http://www.ietf.org/rfc/rfc2396.txt"><i>RFC&nbsp;2396: Uniform Resource Identifiers (URI):
 * Generic Syntax</i></a>, amended by <a
 ...
 * </blockquote>
 * <p>
 * This fragment is not technically part of the URL. Rather, it indicates that after the specified
 * resource is retrieved, the application is specifically interested in that part of the document
 * that has the tag <code>chapter1</code> attached to it. The meaning of a tag is resource specific.
 * <p>
 * An application can also specify a "relative URL", which contains only enough information to reach
 * the resource relative to another URL. Relative URLs are frequently used within HTML pages. For
 * example, if the contents of the URL: <blockquote>
 * 
 * <pre>
 ...
 * must be specified. The optional fragment is not inherited.
 * <p>
 * The URL class does not itself encode or decode any URL components according to the escaping
 * mechanism defined in RFC2396. It is the responsibility of the caller to encode any fields, which
 * need to be escaped prior to calling URL, and also to decode any escaped fields, that are returned
 * from URL. Furthermore, because URL has no knowledge of URL escaping, it does not recognise
 * equivalence between the encoded or decoded form of the same URL. For example, the two URLs:<br>
 * 
 * <pre>
 ...
 * <p>
 * Note, the {@link java.net.URI} class does perform escaping of its component fields in certain
 * circumstances. The recommended way to manage the encoding and decoding of URLs is to use
 * {@link java.net.URI}, and to convert between these two classes using {@link #toURI()} and
 * {@link URI#toURL()}.
 * <p>
 * The {@link URLEncoder} and {@link URLDecoder} classes can also be used, but only for HTML form
...
public final class URL implements java.io.Serializable {
  ...
  /**
   * Creates a <code>URL</code> object from the specified <code>protocol</code>, <code>host</code>,
   * <code>port</code> number, and <code>file</code>.
   * <p>
   *
   * <code>host</code> can be expressed as a host name or a literal IP address. If IPv6 literal
   * address is used, it should be enclosed in square brackets (<tt>'['</tt> and <tt>']'</tt>), as
   * specified by <a href="http://www.ietf.org/rfc/rfc2732.txt">RFC&nbsp;2732</a>; However, the
   * literal IPv6 address format defined in <a
   * href="http://www.ietf.org/rfc/rfc2373.txt"><i>RFC&nbsp;2373: IP Version 6 Addressing
   * Architecture</i></a> is also accepted.
   * <p>
   *
   * Specifying a <code>port</code> number of <code>-1</code> indicates that the URL should use the
   * default port for the protocol.
   * <p>
   *
   * If this is the first URL object being created with the specified protocol, a <i>stream protocol
   * handler</i> object, an instance of class <code>URLStreamHandler</code>, is created for that
   * protocol:
   * <ol>
   * <li>If the application has previously set up an instance of
   * <code>URLStreamHandlerFactory</code> as the stream handler factory, then the
   * <code>createURLStreamHandler</code> method of that instance is called with the protocol string
   * as an argument to create the stream protocol handler.
   * <li>If no <code>URLStreamHandlerFactory</code> has yet been set up, or if the factory's
   * <code>createURLStreamHandler</code> method returns <code>null</code>, then the constructor
   ...
   * </pre>
   * 
   * </blockquote> If the value of that system property is not <code>null</code>, it is interpreted
   * as a list of packages separated by a vertical slash character '<code>|</code>'. The constructor
   * tries to load the class named: <blockquote>
   * 
   * <pre>
   *         &lt;<i>package</i>&gt;.&lt;<i>protocol</i>&gt;.Handler
   * </pre>
   * 
   * </blockquote> where &lt;<i>package</i>&gt; is replaced by the name of the package and
   * &lt;<i>protocol</i>&gt; is replaced by the name of the protocol. If this class does not exist,
   * or if the class exists but it is not a subclass of <code>URLStreamHandler</code>, then the next
   * package in the list is tried.
   * <li>If the previous step fails to find a protocol handler, then the constructor tries to load
   * from a system default package. <blockquote>
   * 
   * <pre>
   *         &lt;<i>system default package</i>&gt;.&lt;<i>protocol</i>&gt;.Handler
   * </pre>
   * 
   * </blockquote> If this class does not exist, or if the class exists but it is not a subclass of
   * <code>URLStreamHandler</code>, then a <code>MalformedURLException</code> is thrown.
   * </ol>
   *
   * <p>
  ...
  /**
   * Creates a URL from the specified <code>protocol</code> name, <code>host</code> name, and
   * <code>file</code> name. The default port for the specified protocol is used.
   * <p>
   * This method is equivalent to calling the four-argument constructor with the arguments being
   * <code>protocol</code>, <code>host</code>, <code>-1</code>, and <code>file</code>.
   *
   * No validation of the inputs is performed by this constructor.
   *
   * @param protocol the name of the protocol to use.
  ...
  /**
   * Creates a <code>URL</code> object from the specified <code>protocol</code>, <code>host</code>,
   * <code>port</code> number, <code>file</code>, and <code>handler</code>. Specifying a
   * <code>port</code> number of <code>-1</code> indicates that the URL should use the default port
   * for the protocol. Specifying a <code>handler</code> of <code>null</code> indicates that the URL
   * should use a default stream handler for the protocol, as outlined for:
   ...
   * <code>NetPermission("specifyStreamHandler")</code> permission. This may result in a
   * SecurityException.
   *
   * No validation of the inputs is performed by this constructor.
   *
   ...
   *            method doesn't allow specifying a stream handler explicitly.
   * @see java.lang.System#getProperty(java.lang.String)
   * @see java.net.URL#setURLStreamHandlerFactory(java.net.URLStreamHandlerFactory)
   * @see java.net.URLStreamHandler
   * @see java.net.URLStreamHandlerFactory#createURLStreamHandler(java.lang.String)
   ...
   * &quot;Uniform Resource Identifiers : Generic * Syntax&quot; : <blockquote>
   * 
   * <pre>
   ...
   * <p>
   *
   * Returns <code>true</code> if this <code>URL</code> and the <code>other</code> argument are
   ...
   *
   * <P>
   * A new instance of {@linkplain java.net.URLConnection URLConnection} is created every time when
   * invoking the {@linkplain java.net.URLStreamHandler#openConnection(URL)
   * URLStreamHandler.openConnection(URL)} method of the protocol handler for this URL.
   * </P>
   *
   * <P>
   * It should be noted that a URLConnection instance does not establish the actual network
   * connection on creation. This will happen only when calling
  ...
  /**
   * Opens a connection to this <code>URL</code> and returns an <code>InputStream</code> for reading
   * from that connection. This method is a shorthand for: <blockquote>
   * 
   * <pre>
  ...
  /**
   * Sets an application's <code>URLStreamHandlerFactory</code>. This method can be called at most
   * once in a given Java Virtual Machine.
   *
   * <p>
   * The <code>URLStreamHandlerFactory</code> instance is used to construct a stream protocol
   * handler from a protocol name.
   *
   * <p>
   * If there is a security manager, this method first calls the security manager's
   * <code>checkSetFactory</code> method to ensure the operation is allowed. This could result in a
   * SecurityException.
   *
   * @param fac the desired factory.
   * @exception Error if the application has already set a factory.
   * @exception SecurityException if a security manager exists and its <code>checkSetFactory</code>
  ...
  private static final boolean enableGopher = AccessController
    ...
    if (handler == null) {
      ...
      if (handler == null) {
        ...
        while (handler == null && packagePrefixIter.hasMoreTokens()) {
          ...
          if (protocol.equalsIgnoreCase(GOPHER) && packagePrefix.equals(JDK_PACKAGE_PREFIX)
              && !enableGopher) {
            continue;
          }
          try {
   ...
   *             calling getURLStreamHandler with the protocol variable returns a valid
